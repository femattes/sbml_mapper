...
def writeDBModelToSBML(database_path, sbml_output_path, modelRow=1):
    ...
    if len(set(expected_tables).difference(set(db_tables))) != 0:
        raise Exception("[Error] writeDBModelToSBML, {0} : Cannot find table(s) {1} in database.".format(database_path, set(expected_tables).difference(set(db_tables))))
    # check model row existence
    rowcount = c.execute("SELECT COUNT (*) FROM Parametrizations").fetchone()[0]
    if modelRow > rowcount:
        raise Exception("[Error] writeDBModelToSBML, {0} : Cannot select row {1} from table Parametrizations with {2} rows. Aborting model export.\n".format(database_path, modelRow, rowcount))
    ...
    for target, in c.execute('SELECT DISTINCT Target FROM Regulations ORDER BY Target').fetchall():
        ...
        o.setTransitionEffect(1) # transitionEffect=1 means 'assignmentLevel'. Required for logical/multi valued models.
        ...
        for context in target_context_list:
            ...
            param = c.execute('SELECT {0} FROM Parametrizations WHERE rowid={1}'.format(context[0], modelRow)).fetchall()[0][0]
            ...
            # else create a Transition.FunctionTerm term with math element
                ...
                for source_index in range(0, len(source_list)):
                    ...
                    source_tstate = source_tstate_list[source_index] # threshold state for this source (e.g. 0 for the first regulator of RafB in K_RafB_012)
    ...
    if document.getNumErrors() > 0:
        raise Exception("[Error] writeDBModelToSBML: Exported SBML model invalid or inconsistant, aborting model export.\n Error Log from libSBML:\n{0}".format(document.getErrorLog().toString()))
